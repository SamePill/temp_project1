        ...
        >
          ...
          >
            ...
            <p class="flex-grow-0 flex-shrink-0 text-sm text-left text-[#777]">
              가입하신 이메일과 휴대폰 번호를 입력하고 인증번호를 입력해주세요.
            </p>
          </div>
        </div>
      ...
      >
        ...
        >
          ...
          >
            ...
            <input
              :class="
                (chkHp ? 'border-[#ddd]' : 'border-[#ff5252]') +
                ' flex-grow-0 flex-shrink-0 text-base font-medium text-left text-[#191919] flex justify-start items-center flex-grow-0 flex-shrink-0 w-[300px] h-[51px] relative overflow-hidden gap-12 p-4 rounded bg-white border'
              "
              type="text"
              placeholder="휴대폰번호"
              v-model="hp"
            ...
            <button
              :class="
                (blSendCertNo
                  ? ' bg-white border-[#ddd] text-[#191919]'
                  : chkHp && hp.length > 0
                  ? ' border-[#1ba494] bg-[#1BA494]  text-white'
                  : 'bg-[#999]  text-white') +
                ' flex-grow-0 flex-shrink-0 text-base text-left text-[#191919] flex justify-center items-center flex-grow-0 flex-shrink-0 w-[125px] h-[51px] relative overflow-hidden gap-2.5 p-2.5 rounded border '
              "
              @click="reqCertNo()"
          ...
          <p
            class="flex-grow-0 flex-shrink-0 text-sm text-left text-[#ff5252]"
            v-show="!chkHp"
          >
        ...
        >
          <input
            ...
            maxlength="6"
            :class="
              (certNoChk ? 'border-[#ddd]' : 'border-[#ff5252]') +
              ' flex-grow-0 flex-shrink-0 text-base font-medium text-left text-[#191919] flex justify-between items-center flex-grow-0 flex-shrink-0 w-[300px] h-[51px] relative overflow-hidden p-4 rounded bg-white border '
            "
            type="text"
            placeholder="인증번호"
            v-model="authNo"
          ...
          <span style="position: absolute; right: 160px" float:right>{{
            countdown
          }}</span>
        </div>
      </div>
