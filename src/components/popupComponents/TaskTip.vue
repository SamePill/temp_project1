<template>
    <div v-if="isShow" style="position:absolute; top:0px; right:-330px; z-index:999; background-color: #fff;" class="w-[320px] rounded-[10px] ">
        <div class="flex flex-col justify-start items-start relative overflow-hidden p-5 rounded-[10px] bg-white border border-[#1ba494]">
            ...
            <div v-if="tipDiv == 0" >
                <p class="text-[14px] text-[#333333] mt-[20px]">서비스 설명</p>
                <div class="flex justify-between pl-[10px]">
                    <li style="width:15px"></li>
                    <p style="width:calc( 100% - 15px )" class="text-[14px] text-[#333333]">큐밋은 클라이언트와 QA 전문업체를 연결시키는 서비스</p>
                </div>
                <p class="text-[14px] text-[#333333] mt-[20px]">범위</p>
                <div class="flex justify-between pl-[10px]">
                    <li style="width:15px"></li>
                    <p style="width:calc( 100% - 15px )" class="text-[14px] text-[#333333]">큐밋 서비스 웹 및 모바일, Admin, API, 선능 테스트, UI/UX, 호환성, 자동화 검증</p>
                </div>
                <p class="text-[14px] text-[#333333] mt-[20px]">우선 순위</p>
                <div class="flex justify-between pl-[10px]">
                    <li style="width:15px"></li>
                    <p style="width:calc( 100% - 15px )" class="text-[14px] text-[#333333]">웹 버전 통합 테스트(호환성, 성능 테스트 필요)</p>
                </div>
            </div>
...
const tip = ref([{title:'주요 담당 업무 작성'
                 ,contents1:'작성이 어려우신가요?<br/>서비스 설명 및 필요하신 검증 범위, 우선순위 등의 내용을 작성해 보세요'
                ,contents2:[{val:'예시)'}
                ]}
])
